...
def fix_openai_api_key():
    """✅ ARREGLO 1: Verificar y cargar OpenAI API Key correctamente"""
    api_key = os.getenv("OPENAI_API_KEY")
    if api_key:
        api_key = api_key.strip('"').strip("'")
        os.environ["OPENAI_API_KEY"] = api_key
        print(f"✅ OpenAI API Key configurada correctamente: sk-...{api_key[-10:]}")
        return True
    else:
...
def forzar_persistencia_contexto(db, conversation, context_data):
    """✅ VERSIÓN CORREGIDA - Persiste en conversations correctamente"""
    ...
    try:
        ...
        if verify_row and verify_row[0]:
            try:
                ...
                print(f"✅ Verificado: Saldo ${saldo:,.0f} guardado en conversations")
            except:
                print("⚠️ Error verificando persistencia")
        
        db.commit()
        print(f"💾 Contexto persistido: {len(context_data)} elementos")
        
    except Exception as e:
        print(f"⚠️ Error forzando persistencia: {e}")
        db.rollback()

def recuperar_contexto_completo_mejorado(db, conversation):
    """✅ SOLUCIÓN CRÍTICA - FORZAR lectura desde conversations"""
    try:
        print(f"🔍 [CRÍTICO] Recuperando contexto para conversación {conversation.id}")
        ...
        if row and row[0]:
            try:
                ...
                print(f"✅ [ÉXITO] Contexto desde conversations")
                print(f"👤 [DATOS] Cliente: {cliente}")
                print(f"💰 [DATOS] Saldo: ${saldo:,.0f}")
        ...
        print("🔄 [FALLBACK] Solo si conversations vacío...")
...
def safe_get_context_data(conversation):
    """
    Obtiene context_data como diccionario de forma segura - VERSIÓN FINAL INTEGRADA
    """
    try:
        print(f"🔍 Intentando recuperar contexto de conversación {conversation.id}")
        ...
        if hasattr(conversation, 'context_data') and conversation.context_data:
            ...
            if isinstance(raw_context, dict):
                print(f"📋 Contexto como dict: {len(raw_context)} elementos")
                return raw_context
            elif isinstance(raw_context, str) and raw_context.strip():
                try:
                    parsed_context = json.loads(raw_context)
                    print(f"📋 Contexto desde JSON: {len(parsed_context)} elementos")
            ...
            if isinstance(raw_context, str) and raw_context.strip():
                try:
                    if raw_context.startswith('{') or raw_context.startswith('['):
                        parsed_context = json.loads(raw_context)
                        print(f"📋 Contexto desde context: {len(parsed_context)} elementos")
                        return parsed_context
                except json.JSONDecodeError:
                    pass
        
        print("📋 Sin contexto en objeto conversation")
        return {}
...
def recuperar_contexto_desde_bd(db, conversation_id):
    """
    Recupera contexto desde la tabla messages cuando mesagge está vacía
    """
    try:
        print(f"🔄 Recuperando contexto desde MESSAGES para conversación {conversation_id}")
        
        from sqlalchemy import text
        
        # Buscar el último mensaje del sistema con información de cliente
        query = text("""
            SELECT TOP 1 
                id,
                text_content,
        ...
        if row:
            ...
            print(f"📋 Mensaje encontrado: {message_content[:100]}...")
            
            # Extraer información del mensaje usando patterns mejorados
            ...
            if saldo_match:
                ...
                try:
                    ...
                    print(f"💰 Saldo extraído: ${saldo_num:,.0f}")
                except ValueError:
                    pass
            
            # Información bancaria (asumir desde datos conocidos)
            if context_reconstructed.get("cliente_encontrado"):
                ...
                # ✅ CALCULAR OFERTAS REALISTAS BASADAS EN SALDO
            ...
            print(f"🔧 Contexto reconstruido desde messages: {len(context_reconstructed)} elementos")
            return context_reconstructed
        else:
            print("❌ No se encontró información de cliente en messages")
            return {}
            
    except Exception as e:
        print(f"⚠️ Error recuperando desde messages: {e}")
        return {}

def forzar_recuperacion_contexto_integrada(db, conversation):
    """
    ✅ VERSIÓN INTEGRADA - Fuerza recuperación de contexto con estrategia híbrida
    """
    try:
        # Primero intentar del objeto conversation
        context_data = safe_get_context_data(conversation)
        
        # Si está vacío, usar estrategia híbrida
        if not context_data or len(context_data) == 0:
            print("🔄 Contexto vacío, usando estrategia híbrida...")
            ...
            if context_data and len(context_data) > 0:
                try:
                    ...
                    print(f"🔧 Contexto restaurado en objeto: {len(context_data)} elementos")
                    
                    # Información del cliente para logs
                    if context_data.get("Nombre_del_cliente"):
                        print(f"👤 Cliente restaurado: {context_data['Nombre_del_cliente']}")
                        print(f"💰 Saldo: ${context_data.get('saldo_total', 0):,.0f}")
                        
                except Exception as e:
                    print(f"⚠️ Error restaurando contexto en objeto: {e}")
        else:
            print(f"✅ Contexto ya disponible: {len(context_data)} elementos")
        
        return context_data
        
    except Exception as e:
        print(f"❌ Error en forzar_recuperacion_contexto_integrada: {e}")
        return {}
...
async def process_chat_message_PATCHED(
    ...
    """
    ✅ VERSIÓN PARCHEADA de tu endpoint existente
    """
    ...
    # ✅ PARCHE 1: Verificar OpenAI al inicio
    openai_disponible = fix_openai_api_key()
    
    try:
        # 1. OBTENER/CREAR CONVERSACIÓN (mantener tu código)
        conversation = _get_or_create_conversation(db, user_id, request.conversation_id)
        
        # ✅ PARCHE 2: Recuperación de contexto mejorada
        ...
        if context_data and len(context_data) > 0:
            ...
            if context_data.get("Nombre_del_cliente"):
                print(f"👤 Cliente en contexto: {context_data['Nombre_del_cliente']}")
        else:
            print(f"📋 Contexto actual: vacío")
        ...
        if context_data.get("saldo_total", 0) > 0:
            ...
            if "pago_flexible" not in context_data:
                context_data["pago_flexible"] = saldo * 0.4  # 60% descuento
            ...
            print(f"✅ pago_flexible agregado: ${context_data['pago_flexible']:,.0f}")
        ...
        # ✅ PARCHE 3: Asegurar que el contexto del resultado se preserve
        ...
        try:
            ...
            print(f"🔧 Variables aplicadas con contexto de {len(contexto_variables)} elementos")
            
        except Exception as var_error:
            print(f"⚠️ Error en variables (usando mensaje original): {var_error}")
            mensaje_con_variables = resultado['message']
        
        # ✅ PARCHE 4: OpenAI solo si está disponible
        mensaje_final = mensaje_con_variables
        if openai_disponible:
            try:
                # Detectar situación empática
                if any(palabra in message_content.lower() for palabra in 
                    ...
                    Genera una respuesta empática y constructiva para alguien en dificultades económicas.
                    Máximo 100 palabras, tono profesional pero comprensivo."""
                    
                    response = client.chat.completions.create(
                        model="gpt-3.5-turbo",
                        messages=[
                            {"role": "system", "content": "Eres un consejero financiero empático."},
                    ...
                    if len(mensaje_empatico) > 20:
                        ...
                        print(f"🤖 Respuesta empática generada con OpenAI")
        ...
        if resultado.get('datos_cliente_encontrados', False):
            ...
            if final_context.get('Nombre_del_cliente'):
                print(f"👤 Cliente: {final_context['Nombre_del_cliente']}")
            if final_context.get('saldo_total'):
                print(f"💰 Saldo: ${final_context['saldo_total']:,.0f}")
        
        # ✅ PARCHE 5: Persistencia forzada de contexto
        forzar_persistencia_contexto(db, conversation, final_context)
        
        # 6. ACTUALIZAR ESTADO (usar método existente)
        conversation = StateManager.update_conversation_state_corregido(
            db=db,
            conversation_id=conversation.id,
            new_state=resultado['next_state'],
            context_data=final_context  # ✅ Usar contexto combinado
        )
        
        # 7. LOG INTERACCIÓN (mantener tu código)
        _log_interaction_integrada(db, conversation, message_content, resultado, request.button_selected)
        
        print(f"✅ Respuesta parcheada generada - Estado: {resultado['next_state']}")
        
        return ChatResponse(
            conversation_id=conversation.id,
            message=mensaje_final,  # ✅ CON VARIABLES + OpenAI OPCIONAL
            current_state=resultado['next_state'],
            buttons=resultado.get('buttons', []),
            context_data=final_context  # ✅ CONTEXTO COMPLETO
        )
...
def _log_interaction_integrada(db, conversation, mensaje, resultado, button_selected):
    """Log de interacciones con información adicional"""
    try:
        # Log mensaje usuario
        ...
        print(f"✅ Interacción logueada correctamente")
...
async def test_variables_endpoint(
    ...
    """Endpoint para probar resolución de variables"""
    try:
...
async def test_openai_endpoint():
    """Endpoint para probar integración OpenAI"""
    try:
        ...
        if not openai_service.disponible:
            return {
                "disponible": False,
                "mensaje": "OpenAI no está configurado. Agregar OPENAI_API_KEY al .env"
            }
        
        # Mensaje de prueba
        mensaje_prueba = "Te ofrezco estas opciones: Pago único de $1,925,138 o plan en 6 cuotas de $481,427 cada una."
...
async def test_cedula_lookup(
    ...
    """Endpoint para probar búsqueda de cédulas directamente"""
    try:
        print(f"🔍 Test de búsqueda para cédula: {request.cedula}")
        ...
        if datos_cliente.get("cliente_encontrado", False):
            ...
            return CedulaTestResponse(
                ...
                mensaje=f"No se encontró información para la cédula {request.cedula}"
            )
            
    except Exception as e:
        print(f"❌ Error en test de cédula: {e}")
        return CedulaTestResponse(
            cedula=request.cedula,
            cliente_encontrado=False,
...
async def get_conversation_history(
    ...
    try:
        print(f"📖 Obteniendo historial de conversación {conversation_id}")
        ...
        print(f"✅ Historial obtenido: {len(messages_formatted)} mensajes (total: {total_messages})")
...
def _get_or_create_conversation(db: Session, user_id: int, conversation_id: Optional[int] = None) -> Conversation:
    """Obtiene o crea conversación con manejo seguro de contexto"""
...
async def test_openai_endpoint():
    """Endpoint para probar integración OpenAI"""
    try:
        # ✅ VERSIÓN SIMPLIFICADA SIN IMPORTS FALTANTES
        api_key = os.getenv("OPENAI_API_KEY")
        if not api_key:
            return {
                "disponible": False,
        ...
        if len(api_key) < 20:
            return {
                "disponible": False,
                "mensaje": "OPENAI_API_KEY parece inválida"
            }
